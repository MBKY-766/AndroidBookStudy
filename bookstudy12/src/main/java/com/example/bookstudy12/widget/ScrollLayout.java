...
import com.example.bookstudy12.R;
...
public class ScrollLayout extends LinearLayout {
    ...
    public ScrollLayout(Context context, @Nullable AttributeSet attrs) {
        ...
        iv_scene.setImageBitmap(mBitmap);
    ...
    private boolean moveView(PointF lastPos, PointF thisPos) {
        ...
        if (Math.abs(params.leftMargin) < iv_scene.getMeasuredWidth()) {
            //还没滚到底，继续滚动
    ...
    private void judgeScroll(PointF lastPos, PointF thisPos) {
        int offsetX = (int) (thisPos.x - lastPos.x);
        if (Math.abs(offsetX) < iv_scene.getMeasuredWidth() / 2) {
            //滚回原处
            mScroller.startScroll(offsetX, 0, -offsetX, 0, 1000);
        } else if (offsetX >= iv_scene.getMeasuredWidth() / 2) {
            //滚到右边
