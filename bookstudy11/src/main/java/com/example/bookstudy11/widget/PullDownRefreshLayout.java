...
import com.example.bookstudy11.R;
import com.example.bookstudy11.util.Utils;

public class PullDownRefreshLayout extends LinearLayout implements PullDownScrollView.ScrollListener, View.OnTouchListener {
    ...
    public PullDownRefreshLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        //触发工具栏变色的临界滑动距离
        ...
        mLinearLayout.postInvalidate();// 立即刷新线性布局(线程安全方式)
        addView(mLinearLayout, 0);//把下拉刷新头部布局添加到最前面
    }

    //刷新完毕，恢复原页面，也就是仍把下拉头部缩了进去
