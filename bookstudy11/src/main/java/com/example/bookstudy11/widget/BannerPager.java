...
public class BannerPager extends RelativeLayout {
    ...
    private Runnable mScroll = new Runnable() {
        @Override
        public void run() {
            int index = vp_banner.getCurrentItem() + 1;//获取下一张广告图的位置
    ...
    public void stop() {
        //移除滚动任务
        mHandler.removeCallbacksAndMessages(mScroll);
    }
    ...
    public void setImage(List<Integer> imageList) {
        ...
        for (int i = 0; i < imageList.size(); i++) {
            //获取图片的资源编号
            int imageResId = imageList.get(i);
            //创建一个图像视图对象
        ...
        // 给翻页视图添加简单的页面变更监听器，此时只需要重写onPageSelected方法
